...
1. **允许在工程初始化阶段创建 `.env` 文件（仅一次）**  
   - 当且仅当以下条件同时满足时，AI 可以创建 `.env` 文件：
     - 项目中 **不存在** `.env` 文件；
     - 当前任务属于**技术架构搭建 / 项目初始化**；
     - 创建行为已在实现方案中明确说明。

2. **创建完成后，`.env` 文件立即进入只读状态**  
   - 自 `.env` 文件首次创建完成起，AI 在任何后续任务中：
     - ❌ 不得新增变量
     - ❌ 不得删除变量
     - ❌ 不得修改变量值
     - ❌ 不得调整变量命名
...
- 所有非敏感、可代码内表达的配置，应优先使用：
  - 配置文件（如 `config/*`）
  - 常量定义（如 `constants.ts`）

- `.env` 的存在不代表其内容可以被演进或重构。
